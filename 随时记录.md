...
## element.getBoundingClientRect 
    获取相对于视窗(就是浏览器左上角的位置),这个位置是动态计算的,会根据滚动位置动态改变,如果要计算相对于页面内容左上角的位置,需要加上
...
让当前分支跟踪远程origin dev分支
git branch -u origin/foo foo
让分支foo跟踪远程origin/foo分支
-u等价于 --set-upstream-to=origin/dev
2. 发现两个分支的分歧点
git-merge-base commit1 commit2
或者使用git log --graph --oneline --all 查看分支图
3. 清除未跟踪的文件
git clean -f -d 
强制删除所有未跟踪文件和文件夹
# 等比例盒子
...
4. a标签的ping属性
5. position: 布局，相当于不脱离文档流的fix固定位置，相对于有滚动条的父元素
6. dom4的新操作方法
      Element#prepend()
      Element#append()
      Element#before()
...
8. 尽量给匿名函数定义函数名，主要应用于写回调函数（更好debug和找错误）
...
# 继承的两种方法
    ...
    function inherits(_super, _sub) {
        let f = function () {

        };
        f.prototype = _super.prototype;
        let proto = new f();
        proto.constructor = _sub;
        _sub.prototype = proto;
    }
    function inherts2(_super, _sub) {
        let a = Object.create(_super.prototype, {
            constructor: {
                value: _sub,
                configuarable: false,
                enumerable: false,
    ...
    inherits(Person, Man);
    // inherts2(Person, Man);
    let man = new Man("zcs", "man");
    console.log(man.constructor)
    man.sayName()


原理：
1. 函数创建时候，有一个prototype指针指向一个原型对象。
2. 使用构造函数创建新对象时候，有一个__proto__属性指向构造函数的原型对象（prototype属性）。
3. 寻找属性时候，先搜索自身的属性，然后搜索__proto__属性的对象，然后再搜索__proto__属性对象的__proto__属性，形成链式访问。

两种实现：
1. 通过create方法，create(a, b)会创建一个对象，该对象的__proto__指向a，然后会把b作为属性附加到新创建的对象上。
b的参数写法同defineProperties {
    value: ,
        configuarable,
        writable,
        enumerable
}
这样创建了一个对象，让子类的prototype指向这个对象。
2. 通过一个空函数，让空函数的prototype指向super的prototype，创建这个空函数的对象，那么这个对象的__proto__指向super的prototype
...
# vh，vw单位的妙用
地址：https://css-tricks.com/fun-viewport-units/?utm_source=CSS-Weekly&utm_campaign=Issue-269&utm_medium=email
1. 响应式布局，所有单位（高，宽，padding，margin，font-size：11px + 0.3vw）都是用vh或vw
2. 固定比例盒子（width: 15vw height: (16/9)vw)
3. 一个巧妙的css自动显示滚动进度的例子，查看收集到的资料里的html文件

# grid中的minmax方法以及,repeat,auto-fit等
地址：https://bitsofco.de/how-the-minmax-function-works/?utm_source=CSS-Weekly&utm_campaign=Issue-269&utm_medium=email
1. minmax(最小值，最大值)，取最小值和最大值之间的数
2. 可用的单位类型
    1. 长度单位
    2. 百分比单位（相对于grid）
    3. 弹性单位fr（总长度-固定单位剩余的空间平分）
    4. max-content 根据内容计算的最大长度，比如一个句子，保持不换行就是max-content
    5. min-content 根据内容计算的最小长度，保持内容不溢出的状态
    6. auto 在最大值和最小值的计算方法仿照max-content和min-content，只不要额外根据min-width和min-height属性来计算。
3. 一个根据容器宽度自动改变单元格数量和大小的例子：
...
# grid规范
1. 如果给gird-line命名为[a]-start,[a]-end,那么将会产生一个grid-area a。

    grid-template-columns: 
        [full-start] minmax(1em, 1fr) 
        [main-start] minmax(0, 40em) [main-end]
        minmax(1em, 1fr) [full-end];
这里给四条线命命名名字，做出了full area和main area。

2. 命名区域会自动创建四条线：grid-row-start grid-row-end grid-col-start grid-col-end
3. grid-column如果只有一个grid-area的名字，那么就会以grid-area对应的的开始和结束区域线作为grid-column-start和grid-column-end（grid-row同理）
4. 如果grid-column或者grid-row的第二个值缺失，如果第一个值是一个用户定义值，那么第二个默认和第一个一样
...
2. 属性选择器
[attr] 有attr属性的元素
[attr=val]attr属性值是val的元素
[attr~=val]attr属性中匹配包含val的值（val被空格分割)
[attr|=val]匹配val或者val-开头的元素（用来处理语言编码）
    [lang|=fr] {
        font-weight: bold;
    }
[attr|^=val]val开头
...
[attr|*=val]字符中包含val
3. 伪类选择器
    1. :active 作用于用户点击到松开的时间
    2. :link visted hover active定义的顺序是lvha
    3. :indeterminate 
        1. checkbox中dom的indeterminate属性为true
        2. 同一个name中，所有radio都未选中（通常用来做表单提示
            input:indeterminate + label {
                color: red
            }
        3. progress元素
    4. :only-of-type和only-child：表示子元素中这个类型只有一个元素和只有一个子元素
    5. 表单相关：
        1. :in-range :out-range匹配input中type是number的元素值在min属性和max属性之间和不在这之间。
        2. :optional :required 表示input或textarea元素中没有require属性和拥有require属性的元素
        3. :invalid :valid 表示input或form元素未通过验证或通过验证
        4. :read-only :read-write 匹配只读元素或可读写元素
        5. :disabled :enabled 匹配不可用，可用元素
    6. :target 匹配跳转到当前锚点的元素，即使锚点元素是display:none也可以生效。参见收集到的资料代码
    7. :fullscreen 匹配全屏模式下的元素，参见收集到的资料代码

# 一个基于:target伪类选择器的纯css弹出框
详情见收集到的资料/代码段

# indeterminateDom属性
radio或者checkbox通过js设置dom属性，会显示一个横杆。表示未决定，通常用于一个checkbox下有许多子box，当子box有一部分选了，一部分没选时候，设置这个属性。

# object-fit让image元素实现和background-image一样的效果(contain,cover,fill, 并控制显示位置)

# react setState同步异步问题
总结：
非react控制的函数，比如ajax回调，setTimeout回调，和浏览器自定义事件函数中调用setState会同步更新。
而通过react控制的事件函数和生命周期函数，调用时候会执行一个方法把isBatchingUpdates设置为true（默认是false），从而异步更新。


1. 从ajax回调和timeout回调中调用setState，同步更新
2. 从浏览器自定义事件中执行setState，同步更新。
2. 不满足以上条件的，从react自己的生命周期方法和事件响应中调用setState，异步更新。

# react中ref可以获得子组件，然后调用子组件的属性和方法


# webpack相关
1. 如果使用CommonsChunkPlugin打包第三方包的话，如果业务代码变了，vendor打包有可能会变，原因有2：
    1. 模块id会变（模块id可以使用 NamedModulesPlugin或HashedModuleIdsPlugin来统一）
    2. chunkid会变（chunkid是每个文件的id，是数字。如果没有使用动态导入的话，vendor的chunkid是0，但是如果使用动态导入，vendor的动态id就会变，可以使用NamedChunksPlugin来固定vendor的chunkid是vendor)

因此CommonsChunkPlugin打包的话，必须使用NamedChunksPlugin + HashedModuleIdsPlugin（NamedModulesPlugin）来进行处理。
2. 打包第三方库使用dll是最好的
3. 动态引入的话，在代码中通过/* webpackChunkName: "lodash" */可以设置chunkName，但是如果使用chunkhash的话，动态引入中会包含chunkId，这个chunkId也会根据import引用顺序改变(比如原来第一个引用，现在变成第二个)，从而造成其他代码改变，导致动态引入的代码hash也变了。（可使用NamedChunksPlugin解决）
4. style-loader在动态导入的时候，有可能会导致FOUC (Flash of unstyled content)，解决方法，原因是js是网络请求的，css也是通过link请求的，挂载到页面的时候，有可能动态引入的js模块在css模块上边，这会导致刷新样式造成的页面闪烁。解决方法是：设置style-loader中的singletong为true，因为这个会把style挂载到head中的style标签。但是这样的问题是sourceMap就无效了，因为都挂到style标签里了。我的优化方法是，在模板中设置一个script标签作为占位符，然后styleloader都append到这个script上面，就不会出现js在上边，link在下面的情况。

# 纯函数的好处
比如一个react函数，如果函数内使用this.props方法的话，如果这个函数要在componentWillReceiveProps方法中执行的话，就会有问题（因为this.pros是旧的，nextProps才是新的）。而如果把这个函数提取出来，只接受函数参数的话，那么它可以放到任何地方使用-只要提供给它正确的参数。不用依赖当前的环境，不用担心别的什么操作把它依赖的东西改变了。

# 高阶组件应用场景之依赖注入
原文：http://krasimirtsonev.com/blog/article/react-the-powerful-higher-order-component-pattern
可以通过高阶组件的方式实现类似context的深层组件获取顶层数据。
    ...
    function titleHoc(component, deps, get) {
        return class extends React.component {
            constructor(props) {
                super(props);
                this.state = deps.map(get);
            }
            render() {
                <component
                    {...this.props}
    ...
    加入gender：
    inject.register("gender", "男");
    const TitleHoc = titleHoc(Title, ["gender"], inject.get);
    <Title name="zcs"> => <TitleHoc name="zcs">

# js单元测试指南
原文(https://github.com/mawrkus/js-unit-testing-guide#dont-write-unnecessary-expectations)
## 单元测试：
测试一个单元的工作：给定一个输入，最终得到一个结果：返回一个值或抛出异常，改变系统状态，发出第三方调用三者之一。
单元测试是独立的并且相互之间是独立的。
单元测试是轻量级测试：可重复，迅速，具有一致性，易写易读。
单元测试也是代码。
他们必须达到被测试代码的一定程度的质量，单元测试代码可以重构来更易维护和可读。
## 好的可测试的代码原则如下
好的单元测试必须在可测试代码的基础上，一些代码编写原则如下：
* 良好的命名和注释（为什么写这段代码，而不是怎么写的）
* dry：不写重复的代码
* 单一职责原则：每一个对象、函数必须集中干一件事情。
* 在同一个组件中保持单一抽象级别（例如，不要使用相同的方法将业务逻辑与较低级别的技术细节混合在一起）
* 最小化组件之间的依赖
* 支持变量可配置，不要硬编码。
* 使用适当的设计模式，尤其是依赖注入-可以分离业务逻辑和对象的创建
* 避免全局变量
...
## 单元测试正确的顺序
1. 先写测试用例，再写代码，让代码设计的更容易测试
2. 只写最少量的代码来实现功能
3. 使用重构机制来提升代码，测试给你自信你新写的代码不会影响现有功能。
4. 在每个循环中清理代码库让代码更易维护，这样更容易经常修改代码并少量增加。
4. 能够快速反馈给开发者，让你知道你的代码不会破坏其他的东西，并且系统走在正确的路上。


## 正确命名测试
单元工作名称-情景/上下文-期望的行为
describe('[单元工作名称]', () => {
  it('should [期望的行为] when [情景/上下文]', () => {
...
## 当使用Tdd，总是以写一个最简单的失败用例开始

## 测试行为而不是内部实现

# 会导致强制重新布局的js操作
https://gist.github.com/paulirish/5d52fb081b3570c81e3a
哪些css属性会触发布局，绘制，组合：https://csstriggers.com/

# 像素怎么渲染到浏览器上的
原文地址：https://aerotwist.com/blog/the-anatomy-of-a-frame/
![](image/main-thread.svg)
![](image/anatomy-of-a-frame.svg)

# chrome-dev-tool发现性能问题performance
原地址（https://developers.google.com/web/tools/chrome-devtools/evaluate-performance/）
1. ctrl+shift+i打开性能面板
2. 点击右边的齿轮图表可以控制限制条件（网速，cpu等）
3. 主要衡量动画性能的条件之一是fps图表
    ![](image/fps-chart.svg)
    1. **如果看见了红色条**，说明性能有问题了。而绿色的条越高，fps越高。

    ![](image/cpu-summary.svg)
    2. fps图表下面是cpu图表，颜色和下面summary饼图含义一致。如果**cpu图表充满了颜色**，表明cpu在记录工程中已经将近耗尽。当你看到cpu长时间这样充满了颜色，需要找出方法来减少工作。

    3. 鼠标悬浮在Fps，cpu或net图表上，显示截图。用来分析动画过程很有用。
    4. 悬停你的鼠标在Frames区域的绿色块上，可以显示那一帧的fps。
4. 快捷键ctrl+shift+p, 输入show render，出现rendering面板，勾选后，可以在页面上实时显示帧数，gpu等信息。
4. 选取一片区域，在最上的面板单击按住拖动选择一片区域。或者通过按键ad移动，ws缩放。   
5. 找到问题所在
    1. 注意summary面板。在原文例子中，render占了大部分时间。性能的艺术是做更少的工作，你的目标就是减少花费在render上的时间。
    2. 展开Main区域会显示一个火焰图展示明细。x轴代表随着时间推移的记录。被一个柱状代表一个事件。一个很宽的柱状代表这个事件用了很长时间。y轴代表调用栈。当你看到堆叠在一起的事件时，意味着上层事件导致了低层事件。
    3. 注意Ｍain面板中黄色的Animation Frame Fired事件如果右上角有**红色的三角**，表示这个事件可能有问题。
    ![](image/animation-frame-fired.png)
    4. 单击Animation Frame Fired事件。Summary页卡中，注意**reaveal**链接。单击能突出显示触发动画帧事件的事件。还要注意最先的**app.update@app.js:94**，单击能跳转到对应的源代码。
    ![](image/forcereflow.png)
    5. 在调用栈app.update下面，如果有很多比较宽的紫色的布局事件出现，并且上边还有红三角，就有可能有强制重新布局的风险（正常的流程是js->样式计算->布局->绘画->合成，而如果在js里做一些样式的改变，然后又读取高度的操作，会强制进行布局）

# 关于强制重绘
应该只在requestAnimationFrame函数内执行写操作。读操作应该在此之外完成，以避免浏览器进行早期布局。

# chrome dev tools中的performance的main中如何解读
横轴是占用的总时间，纵轴是调用栈。
纵轴如何看：
比如程序a调用了b，c，然后c又调用了ef，那么会形成如下的一个调用栈：
[a] [a   ] [a   ] [a      ]
[b] [b][c] [b][c] [b][c   ]
              [e]    [e][f]

但有的时候，会有重复的列存在，有可能是chrome取样失败从而从新取样。

# 如何写出好代码
参考上边的 javascript编码指南部分。
变量：
1. 使用明确意图的变量：var elapsedTimeInDays；
2. 使用能念出发音的变量：public $genymdhms; =》 public $generationTimestamp;
3. 一个概念一个词，比如不要在类A和类B中都定义get和set方法，使用的时候会区分不出来。
4. 函数用动词，类和属性用名词。

函数：
5. 函数越小越好
6. 一个函数只干一件事情
7. 不要嵌套控制结构（比如if下面更多的if或者case）
8. 参数少一些更好，超过3个参数就不好了
9. 函数只做函数名告诉我们的事情（比如getTime函数，就只返回时间，如果还把时间提前一天，就不对了）
10. 避免输出参数

注释
11. 解释你的设计意图：//如果我们在这里排序了数组，那么在calculatePayment（）方法中的逻辑会简单。

不好的例子：
1. 执行不到的死代码
2. 不考虑特殊情况的代码（比如不校验参数，如果参数为空呢）
3. 大类
4. 一个文件中多个语言
5. 更改框架核心功能
6. 不要使用神奇数字-就是程序中直接写一个数字比如：target = 10 * price;要是用变量替代：const taxt = 10; target = 10 * price;
7. if中长判断式（使用函数替代判断式）
8. 调用父类被覆盖的方法。
9. 循环依赖
10. 循环引用
...
4. 空(undefined, nan,null，等)
# js canvas转blob变大问题
1. 使用canvas.toBlobl(mimtype, quality)解决
2. 如果使用toDataUrl的话，有时候会转成png

# 如何单元测试react组件
两篇文章：
https://medium.com/@skidding/testing-react-components-30516bc6a1b3 
按道理应该测试纯展示型组件，然后测试每一个部分，但是作者观点是在实际应用中，涉及大量的
全局变量，ajax等，还有充满了胶水代码的容器组件都应该统一测试，就是使用https://github.com/react-cosmos/react-cosmos这个库来整体测试一个组件的实际使用，而不是每一个小部件。

https://medium.freecodecamp.org/the-right-way-to-test-react-components-548a4736ab22 
重要观念：
对要测试的组件的预期行为以及对其使用情况的合理假设充分了解。然后决定哪些特性需要测试，然后针对性的写测试用例。


我的分析：
1. 只测试无生命周期函数的组件
2. 关注组件渲染什么东西，在不同情况下渲染不同的东西吗等根据条件写测试用例。
...
action描述了发生了什么，reducers描述了状态如何改变。
reducers描述了当action发给store时候如何改变状态。
在状态树中有时候需要存储数据，有时候需要存储ui状态。这很正常，但是要把数据和UI状态分离开来。
序列化数据（变成类似数据库的id关联表）是不错的（可以考虑后续加入）
reducers是纯的，不改变参数，不引用变量，不执行异步等副作用。
考虑使用immutability-helper, updeep, Immutable库来更新state。
1. store全局唯一
2. state是只读的，只能通过action除服
3. state的改变通过纯函数

1. 提取出选择器-从store中获取状态的函数，他们可以用在许多地方：比如Container组件以及ajax回调。 使用reselect：https://github.com/reactjs/reselect
2. 使用duck组织方式：https://github.com/erikras/ducks-modular-redux
3. action命名方式：https://github.com/redux-utilities/flux-standard-action {type,payload,error}

# app 开发最佳实践
1. 分析使用情况：
    1. 用户可操作的：基本上就是action
    2. 用户可见的
2. 确定数据模型，app所需的最小状态（去除可以推导计算的数据），写一个json对象出来
3. 创建所有的action（行为）函数来覆盖所有的用户交互。
4. 写出reducer告诉数据如何改变
5. 写单元测试测试action和reducer。
6. 最后写html-only的界面。
7. 最后添加样式
...
2. 把所有的查询写成函数，然后有别的模块依赖的时候调用这个选择器（依赖倒置），防止直接读别的模块的state
3. action的type前缀加上功能模块名：export const ADD = 'todos/ADD';

redux全家桶
redux   原生redux
react-rudux react和redux集成的
react-redux-router 配合react-router的redux工具
reselect  查询state树
redux-action 创建action和reducer工具，可减少模板代码
redux-thunk redux异步中间件（原生redux只支持同步）
redux-devtools redux开发工具（开发时候调试用，需集成进代码里）
normalizr 把json数据表格化（把统一的数据类型提取出来，放入id比如:
article: {
    user: {
        id: 1,
        name: zcs
...
# ...操作符
1. 传递参数 func(...args) args = [1, 2, 3]
2. 使用new 方法 new People(...args)
3. 数组中：[...a, ...b]
4. 对象: let obj = {...d};
5. 复制数组let ar = [1,2,3]; let copy = [...ar];
6. 连接数组：let a = [1,2,3] let b= [4,5,6]; let concat = [...a, ...b];
7. 复制对象：let a = {c:1} let clone = {...a};
8. 合并对象：let a = {c:1} let b = {d: 2}; let merge = {...a, ...b};

# npm脚本如何将命令行中的参数传递到后续所有的脚本
命令格式`npm run xx --key=value`这个格式中--后会认为是参数,这个会变成一个环境变量npm_config_key
可以通过process.env.npm_config_key获得，值就是value。

# 快速清除数组中的无效项目
`[false, null, undefined, 0, NaN].filter(Boolean)`
因为Boolean是一个构造函数，可以移除false, null, undefined, 0, Na的项目

# 关于npminstal的策略
子包的依赖，如果父亲其他的包没有和这个依赖有冲突（比如同名不同版本），会放到父亲的包。
比如下面的，就会变成a-b,c,d
...
如果有冲突的依赖，就会安装到自己的node_modules下面
...
# vscode技巧
1. 找到所有目标的引用和索引`shift + F12`
2. 改变目标名称以及文件内所有引用的位置`F2`
3. 在git视图，点击进入两栏diff视图后，按`F7`可以在每一段diff代码之间跳跃
4. 列选择 `ctrl + alt + shift + 上下左右`
5. debug中添加log点，`{}`输出变量不中断程序运行，不改变源代码，对于生产环境很有用。
6. debug条件断点：可添加条件断点
7. debug行内断点：可添加行内断点，用于debug压缩的代码
8. debug函数断点：在左下角断点面板点加号，输入函数名。可根据函数名断点，比如你知道某个函数，但是不知道源代码位置。（不管事）
9. 重命名引用的文件名称，import x from './app.js' 在app.js上右键点击重命名引用或F2.

# esProxy的trap内置属性
[参考1](https://ponyfoo.com/articles/es6-proxy-traps-in-depth)
[参考2](https://ponyfoo.com/articles/more-es6-proxy-traps-in-depth)
...
# symbol类型
创建唯一值，用来创建对象的唯一键。可用来在我的替换map中使用

# webpack 代码分离 动态导入
1. 如果应用动态引入,不要设置name，这样打包不是最优解，下面就可以。  

        splitChunks: {
            chunks: 'all',
        },
2. 让打包名称包含文件名：`/* webpackChunkName: "chunk-[request]"*/`

# javascript禁止直接引用变量，方便单元测试
禁止在模块内使用引用索引方式，使用class,添加到static属性或prototype属性，
这样可以通过在jest里直接改class或对象来mock函数内的引用变量,否则无法单元测试。
比如我要测试toTest函数，
...
这个函数以及许多类似的胶水函数，是需要测试的,比如这里需要测如果check执行为true，是否返回{a:1,b:2}, 执行为false，是否返回空对象。对于这种函数，我们需要剥离引用的函数，测试他自己的特性。
但是因为是直接引用的，很难mock。
    ...
    test('test toTest fn when check pass', () => {
        Check.check = jest.fn(() => {return true)
        expect(toTest(3).equal({
            a: 1,
            b: 2
        }))
    })
    test('test toTest fn when check fail', () => {
        Check.check = jest.fn(() => {return false)
        expect(toTest(3).equal({
            a: 1,
            b: 2
        }))
...
# 如何状态管理
## 好的思维方法，概念大于状态管理库
[查看详情](https://medium.freecodecamp.org/how-to-visually-design-state-in-javascript-3a6a1aadab2b)
## 区分存储的数据
1. 业务数据
2. 状态数据
3. ui展示和操作数据
...
1. 提前规划，画出状态图
2. 状态是有限的
3. 根据状态图的规划开发代码，而不是发现状态转换间的限制条件（比如loading状态不能专项adding状态），再去写补丁代码
## 为了解决复杂的状态条件，画状态图表
* 初始状态  - 由带箭头的点标记的起始状态。
* 嵌套状态  - 可以访问其父级转换的状态。
* 并行状态  - 由虚线表示的两个非触摸状态。
...
# Object.defineProperty语法以及根据语法创建双向绑定数据模型
原地址：http://www.hhspace.cn/?p=377
我的优化代码：
    ...
    class Observe {
        ...
        on(path, cb) {
            if (Array.isArray(path) && typeof cb === 'function') {
                const key = path.join('/');
        ...
        emit(path, value, oldValue) {
            if (Array.isArray(path)) {
                ...
                Object.keys(this.events).forEach(p => {
                    if (key.startsWith(p)) {
                        this.events[p].forEach(cb =>
                            cb({ value, oldValue, setPath: key, emitPath: p }),
                        );
                    }
                });
            }
        ...
        init(data) {
            const stack = [];
            const { formatStackData, copyData } = this;
            const toReturValue = copyData(data);
            stack.push(formatStackData(toReturValue));
            const that = this;
            while (stack.length > 0) {
                const curDataInfo = stack.shift();
                const { data: target, path } = curDataInfo;
                for (let pro in target) {
                    let value = (target[pro] = copyData(target[pro]));
                    let curPath = path.concat(pro);
                    if (typeof value === 'object') {
                        stack.push(formatStackData(value, curPath));
                    }
                    Object.defineProperty(target, pro, {
                        get() {
                            console.log('get: ' + curPath.join('/'));
                            return value;
                        },
                        set(toSetValue) {
                            if (typeof toSetValue === 'object') {
                                toSetValue = new Observe(toSetValue);
                            }
                            that.emit(curPath, toSetValue, value);
                            value = toSetValue;
                        },
                    });
                }
    ...
    const data = new Observe(originData);
    data.on(['a'], ({ value, oldValue, setPath, emitPath }) => {
        console.log(`设置路径： ${setPath} 触发路径：${emitPath}`);
...
1. 在标准es6下, function悬置到块的顶端，一开始代码段加'use strict'执行第一样console会报错，因为没有定义，实际悬置如下：

        'use strict'
        function b() {
            {
                let hoistBlock = function f() {}
            }
        }
2. 在非标准情况下，浏览器做了兼容处理，悬置了两次，如下：

        function b() {
            var hoistFunc = undefined;
            // output undefined
            console.log(f);
            {
                let hoistBlock = function f() {}
                // output hoistBlock [Function: f]
                console.log(f);
                hoistFunc = f;
            }
            // output hoistFunc [Function: f]
            console.log(f);
        }
