...
6. keepAliveMsecs:这个参数实际会作用于socket.setKeepAlive,并不是每隔多少时间发送一个嗅探包判断对面是否还保持连接，而是tcp连接由空闲（最后收到数据的时间）开始计时，多少时间后第一次发嗅探包。[github上的讨论](https://github.com/nodejs/node/pull/10702)
