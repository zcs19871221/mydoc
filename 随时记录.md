...
3. 当元素改变值(触发change或input事件的时候)，执行widget.update，遍历models.item中的每一个双向绑定，执行updateModel改变scope的值，然后对每个子组件执行update方法。
4. updateModel方法
...
# 偏重node的js基础知识题（面试提高）
1. 变量赋值是传值，传引用还是什么
首先，变量只是一个指向内存空间的指针，改变赋值不影响内存中的值。
对于基本类型(数字，布尔，字符串，null，undefined)来说，赋值就是除了新建指针外，还把内容也复制一份。
而对于对象来说，赋值就是新建一个指针指向内存。
2. 相等==判断
...
3. 关于buffer
    0. 固定长度，分配后不能变长度
    1. 不受v8内存限制，由底层c++分配
    1. new Buffer目前已经配废弃，实例并没有被初始化，而是保留了原先内存地址上一些随机的值
    2. Buffer.alloc()从内存中分配指定长度的内存空间，并且默认使用 0 值对分配的内存进行填充
4. 关于内存回收
